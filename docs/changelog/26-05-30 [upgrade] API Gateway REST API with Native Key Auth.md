...
`terraform.tfvars` had `restore_api_key = ""` — an empty string. The Restore Lambda read this as `RESTORE_API_KEY`, and every request failed with `[Restore] ❌ Authentication failed: invalid or missing API Key` (403).
...
def lambda_handler(event, context):
    ...
    if not client_key or client_key != RESTORE_API_KEY:
        print("[Restore] ❌ Authentication failed: invalid or missing API Key")
...
def lambda_handler(event, context):
    # The API Gateway REST API is configured with api_key_required = true,
    # so requests without a valid API Key get a 403 from the Gateway before Lambda runs
```
