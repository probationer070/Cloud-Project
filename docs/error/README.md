...
| [ERR-005](ERR-005-claude3-haiku-deprecated-and-iam-glob-mismatch.md) | 2026-06-10 | `project5-document-engine/variables.tf`, `iam.tf` | Query API 500: Claude 3 Haiku retired as Legacy, and after upgrade the IAM glob `anthropic.claude-3-haiku*` failed to match `claude-3-5-haiku`; fixed model ID to 3.5 Haiku and widened glob to `anthropic.claude*` | Resolved (entitlement nuance → ERR-006) |
| [ERR-006](ERR-006-knn-float-mapping-inference-profile-and-model-entitlement.md) | 2026-06-11 | `lambda/query/index.py`, `variables.tf`, `iam.tf`, OpenSearch index | First full query bring-up: kNN 400 (index auto-created with `float` not `knn_vector`), on-demand model ID rejected (Claude 3.5+ needs an inference profile), and model entitlement (3.x Legacy, 4.x Haiku/Sonnet need Marketplace sub) — settled on Opus 4.5 + inference-profile IAM; verified `$2,000,000` answer | Resolved |
